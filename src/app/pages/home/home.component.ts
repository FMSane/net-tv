  ...
  template: `
    <div class="flex h-full overflow-hidden bg-slate-950">
        ...
        <div *ngIf="currentItem; else placeholder" class="aspect-video w-full max-w-6xl relative shadow-2xl bg-black rounded-xl overflow-hidden border border-slate-800 flex flex-col">
             ...
             <div class="h-12 bg-slate-950 border-t border-slate-800 flex items-center justify-end px-4 gap-2 relative z-20">
                ...
                <button (click)="skipForward()" 
                    ...
                    <span class="text-xs font-bold pointer-events-none">+10s</span>
                    <i class="material-icons text-lg pointer-events-none">forward_10</i>
                </button>
             </div>
              ...
              <div class="bg-slate-900 p-4 border-t border-slate-800 flex justify-between items-center z-20 relative">
                ...
                <div class="flex items-center gap-3">
                    <div class="relative group" *ngIf="availableSources.length > 1">
                         <button class="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-700 text-sm transition focus:ring-4 focus:ring-white focus:bg-red-600 outline-none">
                            <i class="material-icons text-sm pointer-events-none">tune</i>
                            <span class="pointer-events-none">{{ currentSource?.name || 'Opciones' }}</span>
                        </button>
                    ...
                    <button (click)="toggleFav(currentItem)" 
                        ...
                        <i class="material-icons pointer-events-none">{{ isFav(currentItem) ? 'star' : 'star_border' }}</i>
                    </button>
                </div>
             </div>
        </div>
      ...
      <aside class="w-[30%] bg-slate-900 flex flex-col h-full border-l border-slate-800 shadow-xl z-30">
        ...
        <div class="p-4 bg-slate-900 shadow-md z-10 flex gap-2 items-center">
            <button (click)="focusSearchInput()" 
                    class="p-3 bg-slate-800 text-white rounded-xl border border-slate-700 hover:bg-red-600 focus:bg-red-600 focus:border-white outline-none transition-colors shrink-0">
                <i class="material-icons pointer-events-none">search</i>
            </button>
            ...
            <div class="relative flex-1">
                <input #searchInput
                       ...
                       placeholder="Buscar..." 
                       class="w-full bg-slate-800 text-white p-3 pr-10 rounded-xl border border-slate-700 outline-none focus:border-red-600 transition-colors">
                
                <button *ngIf="searchText" 
                        (click)="clearSearch()" 
                        class="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 text-slate-400 hover:text-white focus:text-red-500 outline-none rounded-full hover:bg-white/10 cursor-pointer">
                    <i class="material-icons text-xl pointer-events-none">close</i>
                </button>
            </div>
        </div>

        <button (click)="scrollList('up')" 
                class="w-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 py-1 flex justify-center border-b border-slate-700 transition-colors outline-none cursor-pointer">
            <i class="material-icons text-3xl pointer-events-none">keyboard_arrow_up</i>
        </button>

        <div #channelListContainer class="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar scroll-smooth outline-none">
            <div *ngFor="let item of getFilteredList()" 
                 [id]="'channel-' + item.id"
                 (click)="selectChannel(item)"
                 class="p-3 rounded-lg cursor-pointer hover:bg-slate-800 flex items-center gap-3 transition border-2 border-transparent focus:border-white outline-none group"
                 [class.bg-slate-800]="currentItem?.id === item.id">
                 
                 <img [src]="item.image" class="w-10 h-10 object-contain opacity-80 group-hover:opacity-100 bg-white/5 rounded p-1 pointer-events-none">
                 <div class="flex-1 min-w-0 pointer-events-none">
                     <h4 class="text-gray-200 font-bold text-sm truncate group-hover:text-white">{{ item.title }}</h4>
                     <p class="text-xs text-gray-500 group-hover:text-gray-200">{{ item.category }}</p>
                 </div>
                 <i *ngIf="isFav(item)" class="material-icons text-yellow-500 text-xs pointer-events-none">star</i>
            </div>
        </div>

        <button (click)="scrollList('down')" 
                class="w-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 py-1 flex justify-center border-t border-slate-700 transition-colors outline-none cursor-pointer">
...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('channelListContainer') channelListContainer!: ElementRef;
  ...
  onSearchChanged() {
      if (this.channelListContainer) {
          this.channelListContainer.nativeElement.scrollTop = 0;
      }
  }

  // NUEVA FUNCIÓN PARA BORRAR LA BÚSQUEDA
  clearSearch() {
      this.searchText = '';
      this.onSearchChanged();
      this.focusSearchInput(); // Opcional: vuelve a poner el foco en el input al borrar
  }
